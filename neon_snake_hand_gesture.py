...
class HandTracker:
    ...
    def get_hand_direction(self, frame: np.ndarray) -> Tuple[Optional[Direction], np.ndarray]:
        """Detect hand position for directional control"""
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if len(landmarks) > 0:
                    ...
                    # Create deadzone in center
                    deadzone = 80
                    
                    if center_x < frame_center_x - deadzone:
                        direction = Direction.LEFT
                        cv2.putText(debug_frame, "LEFT", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 3)
                    elif center_x > frame_center_x + deadzone:
                        direction = Direction.RIGHT
                        cv2.putText(debug_frame, "RIGHT", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 3)
                    elif center_y < frame_center_y - deadzone:
                        direction = Direction.UP
                        cv2.putText(debug_frame, "UP", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 3)
                    elif center_y > frame_center_y + deadzone:
                        direction = Direction.DOWN
                        cv2.putText(debug_frame, "DOWN", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 3)
                    
                    # Draw hand center and deadzone
                    cv2.circle(debug_frame, (int(center_x), int(center_y)), 15, (255, 255, 0), -1)
                    cv2.rectangle(debug_frame, 
                                (int(frame_center_x - deadzone), int(frame_center_y - deadzone)),
                                (int(frame_center_x + deadzone), int(frame_center_y + deadzone)),
                                (255, 255, 255), 2)
                    
                    # Draw direction zones
                    cv2.putText(debug_frame, "LEFT", (10, frame.shape[0] // 2), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)
                    cv2.putText(debug_frame, "RIGHT", (frame.shape[1] - 100, frame.shape[0] // 2), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)
                    cv2.putText(debug_frame, "UP", (frame.shape[1] // 2 - 20, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)
                    cv2.putText(debug_frame, "DOWN", (frame.shape[1] // 2 - 30, frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)
...
class NeonSnakeGame:
    ...
    def run(self) -> None:
        ...
        print("🐍 NEON SNAKE - Hand Gesture Edition! 🐍")
        print("📋 Instructions:")
        print("🖐️ Use hand position to control snake direction:")
        print("• Move hand to LEFT side of camera = Turn LEFT")
        print("• Move hand to RIGHT side of camera = Turn RIGHT")
        print("• Move hand to TOP of camera = Turn UP")
        print("• Move hand to BOTTOM of camera = Turn DOWN")
        print("\n🎯 Keep your hand visible in the camera!")
        print("⚡ Stay outside the white deadzone in center")
        print("🍎 Eat the pink glowing food to grow")
